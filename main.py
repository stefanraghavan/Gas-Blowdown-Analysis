...
def open_pdf():
    ...
    pdf_path = resource_path('Theory_Assumptions.pdf')
...
def gasPropertyLookup(fluid, property):
    ...
    Tc_list      = gasPropTable['Pc'].tolist()
...
root.title("Ideal Gas Blowdown Analysis")
...
# Dropdown for fluid
fluidlabel = tk.Label(inner_frame, text="Ideal Gas", bg="white",font='Helvetica 11 bold')
...
pdf_button = tk.Button(inner_frame, text="Theory\n&\nAssumptions", command=open_pdf, font='Helvetica 11 bold', height=5, width=18,bg="#A9A9A9")
...
# # Global figure, canvas, and toolbar setup
# fig1, axs = plt.subplots(3, figsize=(5,8))
# canvas = FigureCanvasTkAgg(fig1, master=outer_frame)
# canvas_widget = canvas.get_tk_widget()
# canvas_widget.grid(row=0, column=3, padx=10, pady=10)

# toolbar_frame = tk.Frame(outer_frame)
# toolbar_frame.grid(row=1, column=3, padx=10, pady=10)
# toolbar = NavigationToolbar2Tk(canvas, toolbar_frame)
# toolbar.update()
...
# original_image = Image.open('updatedpic.jpg')  # Replace with your image path
